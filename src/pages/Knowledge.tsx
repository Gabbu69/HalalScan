  ...
  return (
    <div className="flex flex-col h-full mx-auto max-w-md w-full pt-4">
      <div className="px-5 mb-4">
        <h2 className="font-amiri italic text-2xl text-[#1B6B3A] dark:text-green-400 font-bold mb-4">Knowledge Base</h2>
        <div className="bg-white dark:bg-[#1a2e22] rounded-xl p-3 flex items-center gap-2 border border-gray-100 dark:border-gray-800 shadow-sm">
          ...
          <input 
            placeholder="Search E-Numbers or Additives"
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-20 w-full space-y-3">
        {filteredRules.map(item => {
          ...
          return (
            <div key={item.id} className={`bg-white dark:bg-[#1a2e22] rounded-xl overflow-hidden shadow-sm transition-all duration-200 border border-gray-100 dark:border-gray-800 border-l-4 ${borderClass}`}>
              <button
                onClick={() => setExpanded(expanded === item.id ? null : item.id)}
                className="flex flex-row items-center justify-between p-4 w-full text-left bg-white dark:bg-[#1a2e22]"
              >
                <div className="flex-1 pr-4">
                  <h3 className="text-[11px] font-bold text-[#1B6B3A] dark:text-green-400 leading-tight">{item.title}</h3>
                </div>
        ...
        <div className="mt-6">
          <div className="bg-[#1B6B3A]/5 dark:bg-green-900/10 p-4 rounded-2xl border border-[#1B6B3A]/10 dark:border-green-900/30">
            <h4 className="text-[10px] font-bold text-[#1B6B3A] dark:text-green-400 uppercase mb-2 tracking-wider">Guideline Spotlight</h4>
            <p className="text-[9px] sm:text-[10px] text-gray-600 dark:text-gray-300 italic leading-relaxed">
              {useAppStore.getState().madhab === 'General' 
                ? "Checking ingredients allows us to align our consumption with our ethical, dietary, or allergy boundaries."
                : '"Everything is Halal until proven otherwise, except in the case of meat, where it is Haram until proven Halal."'}
            </p>
          </div>
        </div>
      </div>
